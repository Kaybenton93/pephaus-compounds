...
export default function Shop() {
  return (
    <main className="page">
        ...
        <div className="product-grid">
          {compounds.map((c) => (
            <article className="card" key={c.slug}>
              ...
              <Link className="btn" href={`/shop/${c.slug}`}>
                VIEW PRODUCT
              </Link>
            </article>
          ))}
      ...
      <section className="shell panel shop-detail">
        <aside className="side">
          <h3>COMPOUNDS</h3>
          {compounds.map((c) => (
            <Link href={`/shop/${c.slug}`} key={c.slug}>
              {c.name}
              <br />
        ...
        <div className="info">
          <h2>
            <span className="red">COMPOUND</span> DETAILS
          </h2>
          <p className="muted">
            Select any compound to view details, COA access, and product information.
